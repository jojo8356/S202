...
> **Stratégie :** sprint *par version* (squelette du Plan A — la notation l'impose), mais à l'intérieur de chaque sprint on **parallélise** entre équipiers (graine du Plan B). Un sprint = un tag = un push. Le chef de projet `[CP]` est *coordinateur*, pas codeur (sauf sur les rôles techniques qui lui sont aussi attribués).

## 3. Répartition consolidée par équipier

| Équipier | Rôle | Périmètre principal | Charge estimée |
| --- | --- | --- | :---: |
...
> Si un équipier veut échanger une tâche, c'est **avant lundi** ; ensuite la répartition est figée. Le Sprint 6 est entièrement assuré par Johan, les autres équipiers n'ont pas à s'en occuper côté code (mais on se retrouve tous pour la soutenance et la répartition des points en S7.2).
...
- `[Nom]` = équipier responsable. `[Johan/CP]` = Johan en casquette chef de projet, `[Johan/D4]` = Johan en casquette tests/review.
...
> Constitution équipe + outillage. À ce stade, les TP Python individuels sont déjà rendus. **Pris en charge par Theo.**

### S0.1 Constitution de l'équipe `[Theo]` · t≈ 1 h
...
### S0.2 Mise en place GitLab IUT `[Theo]` · t≈ 1 h
...
### S0.3 Discipline git documentée `[Theo]` · t≈ 30 min
...
### S0.4 Lecture commune du cahier des charges `[Theo]` + tous · t≈ 1 h
...
### S1.3 Attributs et constructeur `IndexedPage(String[] lines)` `[Haithem]` · t≈ 1 h · 🔗 S1.2
...
### S1.4 Constructeur `IndexedPage(String text)` `[Haithem]` · t≈ 45 min · 🔗 S1.3
...
### S1.7 Tests `SearchEngineTests` `[Mai]` · t≈ 2 h · 🔗 S1.6
...
### S1.8 Code review croisée `[Johan/D4]` · t≈ 1 h · 🔗 S1.7
- ☐ Ouvrir une MR `v0.1` → relue par Johan en casquette D4.
- ☐ Vérifier indentation, javadoc minimale (1 phrase par méthode publique), pas de `printf` oublié.
- ☐ Approuver et merger dans `v0.1`.

### S1.9 Tag, push, livraison `[Johan/CP]` · t≈ 15 min · 🔗 S1.8
...
### S2.2 Constructeur `IndexedPage(Path path)` `[Haithem]` · t≈ 1 h · 🔗 S2.1
...
### S2.6 `launchRequest` + `printResults` `[Mai]` · t≈ 2 h · 🔗 S2.4 + S2.3
...
### S2.7 Résolution INDEX classpath‑safe `[Haithem]` · t≈ 30 min · 🔗 S2.6
...
### S2.8 Méthode `main` (one‑shot + interactif) `[Mai]` · t≈ 1 h · 🔗 S2.6
...
### S2.9 Tests v0.2 `[Johan/D4]` · t≈ 1 h 30 · 🔗 S2.8
...
### S2.10 Code review + tag `[Johan/CP]` · t≈ 30 min · 🔗 S2.9
- ☐ MR mergée dans `v0.2`.
...
> *Vous n'êtes plus guidés.* Choix techniques à documenter dans `doc/decisions_v0.3.md` à mesure (tenu par Johan).

### S3.1 Branche `v0.3` `[Johan/CP]` · t≈ 5 min · 🔗 v0.2

### S3.2 Chargement du dictionnaire `[Haithem]` · t≈ 1 h
...
### S3.3 Chargement de la blacklist `[Haithem]` · t≈ 30 min · 🔗 S3.2
...
### S3.6 Décisions documentées `[Johan/CP]` · t≈ 30 min · 🔗 S3.4
...
### S3.7 Tests v0.3 `[Mai]` · t≈ 1 h 30 · 🔗 S3.5
- ☐ Test : `pommes` ≡ `pomme`.
...
### S3.8 Code review + tag v0.3 `[Johan/CP]` · t≈ 30 min
- ☐ MR mergée.
...
### S4.1 Branche `v0.4` + import du gros corpus `[Johan/CP]` · t≈ 30 min · 🔗 v0.3
- ☐ Décompresser `INDEX_FILES.7z` dans `bin/INDEX/` (sans le commiter — `.gitignore`).
- ☐ Documenter l'emplacement attendu dans `README.md`.

### S4.2 Migration `String[] words / int[] counts` → `HashMap<String,Integer>` `[Haithem]` · t≈ 3 h · 🔗 S4.1
...
### S4.4 Bench séquence vs parallèle vs deux structures `[Mai]` · t≈ 3 h · 🔗 S4.3
...
### S4.5 Profilage mémoire `[Johan/D4]` · t≈ 1 h 30 · 🔗 S4.4
...
### S4.7 Code review + tag v0.4 `[Johan/CP]` · t≈ 30 min
...
> **Sprint pris en charge intégralement par Johan en solo.** Choix d'une seule piste parmi A / B / C ci‑dessous, à valider avec l'enseignant **avant le 5 juin** si on diverge des suggestions du CDC. Le reste de l'équipe n'a pas à intervenir côté code, mais participe à la soutenance et à la répartition des points (S7.2).

### S6.0 Choix de la piste `[Johan]` · t≈ 1 h · 🔗 v0.5
- ☐ Décider entre A (indexation à froid), B (autocorrection) ou C (recherche littérale).
- ☐ Mailer l'enseignant si proposition perso.
  🏁 Décision actée dans `doc/decisions_finales.md`.

### S6.A — Si « Indexation à froid » `[Johan]` · t≈ 12 h
...
### S6.B — Si « Autocorrection » `[Johan]` · t≈ 8 h
...
### S6.C — Si « Recherche littérale `"…"` » `[Johan]` · t≈ 6 h
...
### S6.X Restitution `[Johan]` · t≈ 4 h
- ☐ Rédiger `doc/rapport_final.md` : démarche, choix, résultats, échecs et apprentissages.
- ☐ Préparer 8 slides pour la soutenance.
- ☐ Faire circuler les slides à l'équipe pour la rehearsal commune.

### S6.Y Tag final `[Johan/CP]` · t≈ 15 min
...
### S7.1 Préparation démo `[équipe complète]` · t≈ 2 h
...
- ☐ Rehearsal interne : chacun (Haithem, Mai, Theo, Johan) présente *une* partie.

### S7.2 Répartition des points `[Johan/CP] + équipe` · t≈ 1 h
...
| Équipier | Rôle dominant | Sprints concernés | Charge totale estimée |
| --- | --- | --- | :---: |
